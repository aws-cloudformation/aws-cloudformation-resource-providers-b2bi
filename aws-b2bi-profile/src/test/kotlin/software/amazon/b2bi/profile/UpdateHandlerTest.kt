...
class UpdateHandlerTest : AbstractTestBase() {
    ...
    fun handleRequest(testArgs: TestArgs) {
        every { b2BiClient.updateProfile(any<UpdateProfileRequest>()) } returns testArgs.apiResponse
    ...
    fun handleRequest_throwsException() {
        every {
            proxyClient.client().updateProfile(any<UpdateProfileRequest>())
        } throws AwsServiceException.builder().build()

        val request = ResourceHandlerRequest.builder<ResourceModel>()
            .desiredResourceState(TEST_UPDATE_PROFILE_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS)
            .build()
    ...
    data class TestArgs(
        ...
        val apiResponse: UpdateProfileResponse,
    ...
    companion object {
        ...
        fun updateHandlerSuccessTestData() = listOf(
            TestArgs(
                testName = "Update profile with all updatable fields.",
                requestDesiredResourceModel = TEST_UPDATE_PROFILE_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS,
                requestPreviousResourceModel = ResourceModel(),
                requestDesiredResourceTags = emptyMap(),
                requestPreviousResourceTags = emptyMap(),
                apiResponse = TEST_UPDATE_PROFILE_RESPONSE_WITH_ALL_FIELDS,
                expectedResourceModel = TEST_UPDATE_PROFILE_RESPONSE_RESOURCE_MODEL_WITH_ALL_FIELDS
            )
        )
    }
}

...
class UpdateHandlerTest : AbstractTestBase() {
    ...
    fun handleRequest(testArgs: TestArgs) {
        every { b2BiClient.updatePartnership(any<UpdatePartnershipRequest>()) } returns testArgs.apiResponse
    ...
    fun handleRequest_throwsException() {
        every {
            proxyClient.client().updatePartnership(any<UpdatePartnershipRequest>())
        } throws AwsServiceException.builder().build()

        val request = ResourceHandlerRequest.builder<ResourceModel>()
            .desiredResourceState(TEST_UPDATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS)
            .build()
    ...
    data class TestArgs(
        ...
        val apiResponse: UpdatePartnershipResponse,
    ...
    companion object {
        ...
        fun updateHandlerSuccessTestData() = listOf(
            TestArgs(
                testName = "Update partnership with all updatable fields.",
                requestDesiredResourceModel = TEST_UPDATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS,
                requestPreviousResourceModel = ResourceModel(),
                requestDesiredResourceTags = emptyMap(),
                requestPreviousResourceTags = emptyMap(),
                apiResponse = TEST_UPDATE_PARTNERSHIP_RESPONSE_WITH_ALL_FIELDS,
                expectedResourceModel = TEST_UPDATE_PARTNERSHIP_RESPONSE_RESOURCE_MODEL_WITH_ALL_FIELDS
            )
        )
    }
}
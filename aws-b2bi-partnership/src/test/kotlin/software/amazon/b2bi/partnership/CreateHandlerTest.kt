...
class CreateHandlerTest : AbstractTestBase() {
    ...
    fun handleRequest(testArgs: TestArgs) {
        every { b2BiClient.createPartnership(any<CreatePartnershipRequest>()) } returns testArgs.apiResponse
    ...
    fun handleRequest_throwsException() {
        every {
            proxyClient.client().createPartnership(any<CreatePartnershipRequest>())
        } throws AwsServiceException.builder().build()

        val request = ResourceHandlerRequest.builder<ResourceModel>()
            .desiredResourceState(TEST_CREATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS)
            .build()
    ...
    data class TestArgs(
        ...
        val apiResponse: CreatePartnershipResponse,
    ...
    companion object {
        ...
        fun createHandlerSuccessTestData() = listOf(
            TestArgs(
                testName = "Create partnership with all fields.",
                requestResourceModel = TEST_CREATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS,
                apiResponse = TEST_CREATE_PARTNERSHIP_RESPONSE_WITH_ALL_FIELDS,
                expectedResourceModel = TEST_CREATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_ALL_FIELDS.toBuilder()
            ...
            TestArgs(
                testName = "Create partnership with only required fields.",
                requestResourceModel = TEST_CREATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_REQUIRED_FIELDS,
                apiResponse = TEST_CREATE_PARTNERSHIP_RESPONSE_WITH_REQUIRED_FIELDS,
                expectedResourceModel = TEST_CREATE_PARTNERSHIP_REQUEST_RESOURCE_MODEL_WITH_REQUIRED_FIELDS.toBuilder()
                    .partnershipId(TEST_PARTNERSHIP_ID)
                    .partnershipArn(TEST_PARTNERSHIP_ARN)
                    .createdAt(TEST_INSTANT.toString())
                    .build()

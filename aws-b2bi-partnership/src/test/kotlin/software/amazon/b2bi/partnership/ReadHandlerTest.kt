...
class ReadHandlerTest : AbstractTestBase() {
    ...
    fun handleRequest(testArgs: TestArgs) {
        every { b2BiClient.getPartnership(any<GetPartnershipRequest>()) } returns testArgs.apiResponse
    ...
    fun handleRequest_throwsException() {
        every {
            proxyClient.client().getPartnership(any<GetPartnershipRequest>())
        } throws AwsServiceException.builder().build()

        val request = ResourceHandlerRequest.builder<ResourceModel>()
            .desiredResourceState(TEST_GET_PARTNERSHIP_REQUEST_RESOURCE_MODEL)
            .build()
    ...
    data class TestArgs(
        ...
        val apiResponse: GetPartnershipResponse,
    ...
    companion object {
        ...
        fun readHandlerSuccessTestData() = listOf(
            TestArgs(
                testName = "Read partnership with all fields.",
                requestResourceModel = TEST_GET_PARTNERSHIP_REQUEST_RESOURCE_MODEL,
                apiResponse = TEST_GET_PARTNERSHIP_RESPONSE_WITH_ALL_FIELDS,
                apiTags = TEST_LIST_TAGS_FOR_RESOURCE_RESPONSE,
                expectedResourceModel = TEST_GET_PARTNERSHIP_RESPONSE_RESOURCE_MODEL_WITH_ALL_FIELDS
            )
        )
    }
}